...
Use the repository's existing scanner when available, with full redaction enabled.
For example, [Gitleaks](https://github.com/gitleaks/gitleaks) distinguishes file
scanning from Git history scanning; consult the installed version's help and
record the mode used. Otherwise use local searches for the categories below.
Configure searches to return filenames, line numbers, and categories rather than
matching values or surrounding lines. Do not paste unredacted scanner reports,
matching source lines, or credentials into chat, logs, commits, or external tools.
...
This Skill is an audit procedure, not a bundled secret-detection engine. Its
development format check is `uv run --project /path/to/secret-lint --locked skills-ref validate /path/to/secret-lint`.
