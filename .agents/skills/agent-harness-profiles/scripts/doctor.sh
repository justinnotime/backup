...
command -v sqlite3 >/dev/null 2>&1 || warn 'sqlite3 is unavailable; OpenCode databases use the fallback copy path'

if bash -n "${REPO_ROOT}/backup.sh"; then
  ok 'backup.sh syntax'
else
  error 'backup.sh syntax'
fi
...
if [[ -f "${config_file}" ]]; then
  ...
  warn "configuration not found; native defaults remain available: ${config_file}"
fi
...
if [[ -L "${shared_target}" && "$(readlink -f -- "${shared_target}" 2>/dev/null || true)" == "${SKILL_DIR}" ]]; then
  ok 'shared Agent Skills link'
else
  warn 'shared Agent Skills link is not installed from this checkout'
fi

backup_target="${HOME}/bin/backup"
if [[ -L "${backup_target}" && "$(readlink -f -- "${backup_target}" 2>/dev/null || true)" == "${REPO_ROOT}/backup.sh" ]]; then
  ok 'stable backup command link'
elif [[ -x "${backup_target}" ]]; then
  warn 'backup command exists but is not linked to this checkout'
else
