...
validate_entry() {
  ...
  [[ "${label}" =~ ^[a-z0-9][a-z0-9_-]*$ ]] ||
    fail "${tool} label is unsafe: ${label}"
  [[ "${path}" == /* && "${path}" != / ]] ||
...
render() {
  printf '%s\n' '# Generated by agent-harness-profiles. Edit the local config, then regenerate.'
  printf '%s\n\n' '# The plain commands continue to select their native default roots.'
